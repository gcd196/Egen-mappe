...
class RamseyModelClass():
    ...
    def find_steady_state(self,KY_ss,do_print=True):
        ...
        # a. find A
        ss.K = KY_ss
        #Find the production, real interest rate and wage, where gamma is forced to be 1
        Y,_,_ = production(par,1.0,ss.K)
        #Y is not equal to 1 here. Therefore, gamma will also not be equal to 1. 
        print(f'Y_temp={Y:.4f}')
        #This forces Y to be 1. See the production function. 
        ss.Gamma = 1/Y

        # b. factor prices
        ss.Y,ss.rk,ss.w = production(par,ss.Gamma,ss.K)
        #We know it is close to 1, but good to assert it anyway. 
    ...
    def evaluate_path_errors(self):
        ...
        # a. consumption. Adding the steady state value of C to the path. Right now it is the same value, 0.8 all the time. 
        C = path.C
        #Length of path.C minus 1, where we insert the steady state value. We do not want the first value. 
        C_plus = np.append(path.C[1:],ss.C)
        
        # b. capital - add the lagged value of K. 
        K = path.K
    ...
    def solve(self,do_print=True):
        ...
        def eq_sys(x):
            ...
            #First row is C, the second one is K
            path.C[:] = x[0,:]
            path.K[:] = x[1,:]
            
            # ii. return errors. These must be 0. Therefore, we change the values until this is the case. 
            return self.evaluate_path_errors()
        # b. initial guess. The initial guess is that x0 is equal to the steady state. 
        ...
        # d. final evaluation. We take the input from the solution and give it to the path.C and path.K. Hereby it will change values
        # 
        eq_sys(x)
